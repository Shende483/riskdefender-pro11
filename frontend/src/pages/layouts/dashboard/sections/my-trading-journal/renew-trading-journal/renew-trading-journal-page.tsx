...
import TradingJournalPlanService from "../../../../../../Services/api-services/plan-info-service/my-trading-journal-plan-service";
import { PlanType, CouponResponse } from "../../../../../../Services/api-services/plan-info-service/my-trading-journal-plan-service";
...
export default function TradingJournalPlanPage() {
  const [tradingJournals, setTradingJournals] = useState<TradingJournal[]>([]);
  ...
  useEffect(() => {
    const fetchTradingJournals = async () => {
      ...
      try {
        const response: JournalResponse = await TradingJournalService.getTradingJournalLimits();
        if (response.statusCode === 200 && response.success) {
          setTradingJournals(Array.isArray(response.data) ? response.data : []);
          if (response.data.length === 0) {
            setResponseMessage({ text: "No trading journal data found", type: "info" });
            setShowSnackbar(true);
          }
        } else {
          setResponseMessage({ text: response.message || "Failed to fetch trading journal data", type: "error" });
          setShowSnackbar(true);
        }
      } catch (error) {
        console.error("Error fetching trading journal data:", error);
        setResponseMessage({ text: "Failed to fetch trading journal data", type: "error" });
        setShowSnackbar(true);
      } finally {
  ...
  useEffect(() => {
    const loadPlans = async () => {
      try {
        const planData = await TradingJournalPlanService.getActivePlan();
        if (planData.statusCode === 200 && planData.success) {
          const mappedPlans: PlanType[] = planData.data.map((p: any) => ({
            ...
            tradingJournalLimit: p.tradingJournalLimit ?? 0,
  ...
  const handleJournalSelection = (journalId: string) => {
    setSelectedJournalId((prev) => (prev === journalId ? null : journalId));
  };
  ...
  const handleCouponVerify = async () => {
    ...
    try {
      const response: CouponResponse = await TradingJournalPlanService.verifyCoupon(couponCode);
      const selectedPlan = plans.find((p) => p._id === planId);
      if (!selectedPlan) {
        setResponseMessage({ text: "Please select a plan first", type: "error" });
        setShowSnackbar(true);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!selectedJournalId) {
      setResponseMessage({ text: "Please select a trading journal", type: "error" });
      setShowSnackbar(true);
    ...
    <Card sx={{ py: 3, my: 3, width: "100%", mx: "auto", boxShadow: 3, borderRadius: 3 }}>
      <Box sx={{ mx: 3, mb: 3, display: "flex", gap: 3, flexDirection: "column" }}>
        ...
        <Card sx={{ boxShadow: 2, borderRadius: 2 }}>
          <CardContent>
            <Typography variant="h5" fontWeight={600} mb={2}>
              Trading Journals (Total: {tradingJournals.length})
            </Typography>
            {isLoadingJournals ? (
              ...
              >
                {tradingJournals.map((journal) => (
                  <ListItem
                    key={journal._id}
                    dense
                    onClick={() => handleJournalSelection(journal._id)}
                    sx={{ cursor: "pointer", "&:hover": { bgcolor: "" } }}
                  >
                    <ListItemIcon>
                      <Checkbox
                        edge="start"
                        checked={selectedJournalId === journal._id}
                        disabled={!!selectedJournalId && selectedJournalId !== journal._id}
                      />
                    </ListItemIcon>
                    <ListItemText
                      primary={`Journal ID: ${journal._id}`}
                      secondary={
                        <>
                          <strong>Limit:</strong> {journal.tradingJournalLimit} entries |{" "}
                          ...
                          <strong>Status:</strong> {journal.paymentStatus}
                        </>
                      }
                    />
                  </ListItem>
                ...
                return (
                  ...
                  >
                    ...
                    <Typography variant="body2" color="text.primary" mb={1}>
                      • Additional Limit: {plan.tradingJournalLimit} entries
                    </Typography>
            ...
            >
              ...
              {isFormValid ? (
                <PaymentGateway
                  ...
                  paymentType="tradingJournalRenew"
                  data={{ journalId: selectedJournalId || "" }}
                />
              ) : (
                <Button
                  fullWidth
